...
In Relational databases, the data is stored in table format grouped into rows and columns. SQL is the standard language used to manage databases and the data within them. As a query language, SQL is optimized for the sole purpose of querying data whether it's writing queries or retrieving data.

2. Why do tables need a `primary key`?
Tables need a primary key because it is used to uniquely indentify each row.
...
In order to have a MANY to MANY relationship between two tables, we need to introduce an intermediary table that holds foreign keys that referencee the primary key t=on the related tables.
